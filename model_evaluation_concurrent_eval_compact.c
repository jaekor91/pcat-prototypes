...
#define BLOCK_LOGLIKE (BLOCK + (2 * MARGIN)+ REGION) // BLOCK_LOGLIKE is the region over which log like is computed.
...
#define EVAL_COMPACT 0 // If 1, all (inner) blocks are computed at the same time (set BLOCK_LOGLIKE = BLOCK). If 0, alternation is applied. 
...
{	
	...
	int num_blocks_computed;
	if (EVAL_COMPACT){
		num_blocks_computed = (NUM_BLOCKS_PER_DIM * NUM_BLOCKS_PER_DIM);
	}
	else{
		num_blocks_computed = (NUM_BLOCKS_PER_DIM * NUM_BLOCKS_PER_DIM /4);		
	}	
	...
	for (i=0; i<size_of_nstar; i++){
		...
		for (j=0; j<NITER; j++){
			...
			// EVAL_COMPACT: Changes which blocks are evaluated and how
			int increment = 2;
			if (EVAL_COMPACT) 
			{
				par_Y = 0;
			...
			#pragma omp parallel
			{
				...
				for (iby=1+par_Y; iby< (NUM_BLOCKS_PER_DIM_W_PAD-1); iby+=increment){ // Column direction				
					for (ibx=1+par_X; ibx< (NUM_BLOCKS_PER_DIM_W_PAD-1); ibx+=increment){ // Row direction
						...
						for (k=0; k<jstar; k++){
							if (EVAL_COMPACT){
								idx_row = MARGIN + p_X[k];
								idx_col = MARGIN + p_Y[k];	
							}
							else{
	...
	double dt_eff = dt_per_iter/ num_blocks_computed;
